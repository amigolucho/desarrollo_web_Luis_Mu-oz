...
def get_comments(aviso_id):
	conn = get_conn()
	cursor = conn.cursor()
	sql = "SELECT * FROM comentario WHERE aviso_id = %s ORDER BY fecha DESC"
	cursor.execute(sql, (aviso_id))
...
def add_comment(nombre, texto, fecha, aviso_id):
	conn = get_conn()
	cursor = conn.cursor()
	sql = "INSERT INTO comentario (nombre, texto, fecha, aviso_id) VALUES (%s, %s, %s, %s)"
	cursor.execute(sql, (nombre, texto, fecha, aviso_id))
	conn.commit()